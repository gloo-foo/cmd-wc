...
func columns(f flags, c counts) []column {
	shouldShowAll := !bool(f.linesEnabled) && !bool(f.wordsEnabled) && !bool(f.bytesEnabled) &&
		!bool(f.charsEnabled) && !bool(f.maxLineLengthEnabled)
	return []column{
		{isEnabled: shouldShowAll || bool(f.linesEnabled), value: c.lines},
		{isEnabled: shouldShowAll || bool(f.wordsEnabled), value: c.words},
		{isEnabled: shouldShowAll || bool(f.bytesEnabled), value: c.bytes},
		{isEnabled: bool(f.charsEnabled), value: c.chars},
		{isEnabled: bool(f.maxLineLengthEnabled), value: c.maxLen},
	}
}
