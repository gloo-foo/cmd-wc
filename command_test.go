...
func TestWc_MultiLine(t *testing.T) {
	...
	lines, err := testable.TestLines(command.Wc(), run.Input(input))
	assertion.NoError(t, err)
...
func TestWc_NonOptionArgumentIsClassifiedNotFolded(t *testing.T) {
	// A positional argument (a File-convertible string) is not one of wc's
	// options: it must pass through the option fold untouched, leaving every
	// flag at its default, and the command still counts the piped stream.
	lines, err := testable.TestLines(command.Wc("ignored.txt"), "hello world\nfoo bar\n")
	assertion.NoError(t, err)
	assertion.Lines(t, lines, []string{"2 4 18"})
}
...
func TestWc_TableDriven(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			lines, err := testable.TestLines(command.Wc(tt.opts...), run.Input(tt.input))
			assertion.NoError(t, err)
